...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (targetUser) {
            ...
            const deleted = await channel.bulkDelete(userMessages, true).catch(err => {
                console.error(`Error deleting messages from user ${targetUser.id}:`, err.message);
                return null;
            });
            ...
            const deleted = await channel.bulkDelete(amount, true).catch(err => {
                console.error(`Error bulk deleting ${amount} messages:`, err.message);
                return null;
            });
            deletedSize = deleted ? deleted.size : 0;
        }
