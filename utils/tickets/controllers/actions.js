 ...
 */
const { PermissionsBitField, ChannelType, MessageFlags, AttachmentBuilder } = require('discord.js');
...
// Directorio para guardar transcripts HTML
...
const INACTIVITY_MS = config.tickets.inactivityWarningMs;
const INACTIVITY_RETRY_MS = config.tickets.inactivityRetryMs;
...
async function executeClose(interaction, ticket) {
    ...
    try {
        ...
        // Generar transcript como HTML (obtener string directo con todos los estilos)
        ...
        // Crear buffer para attachment de Discord
        const htmlBuffer = Buffer.from(htmlString, 'utf-8');
        const transcriptAttachment = new AttachmentBuilder(htmlBuffer, { 
            name: `transcript-${channel.name}.html` 
        });

        // Guardar URL relativa en la BD
        const transcriptUrl = `/transcript/${ticket.ticketId}`;
        ...
        // Actualizar ticket con URL del transcript
        await prisma.ticket.update({
            where: { ticketId: ticket.ticketId },
            data: { transcriptUrl: fileName }
        });
