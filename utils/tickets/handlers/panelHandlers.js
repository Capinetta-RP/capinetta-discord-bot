...
async function handleSendPanel(interaction) {
    ...
    await interaction.reply({ 
        content: `Vas a publicar el panel en ${targetChannel}. Confirmá tu acción:`, 
    ...
    const collected = await msg.awaitMessageComponent({ filter, max: 1, time: 15000 }).catch(() => null);

    if (!collected) {
        return interaction.editReply({ content: 'Operación cancelada por timeout.', components: [] });
    }

    if (collected.customId === 'cancel_panel') {
        return collected.update({ content: '❌ Operación cancelada.', components: [] });
    }
    ...
    return collected.update({ content: "✅ Panel enviado y vinculado para actualizaciones automáticas.", components: [] });
}
